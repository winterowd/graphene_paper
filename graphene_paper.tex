...
We have measured the time-reversal odd condesnsate on the lattice ensembles with the largest flux $\Phi_B=0.125$ for $a_{\tau} T = [0.002, 0.016]$. These values of $a_{\tau}T$ are smaller than the dynamical fermion mass, as one can see from Fig. \ref{MFvsm}. Thus, one expects that if the ground state did support a Haldane condensate, these temperatures would be sufficiently small and the magnetic flux sufficiently large to observe this. What we have found is that our data does not support the appearance of a Haldane condensate. In Fig. \ref{ReHaldanevsm},  we have plotted the real part of the Haldane mass, computed with $1000$ stochastic sources on $100$ gauge configurations, versus $m$ for $N_{\tau}=480,~\Phi_B=0.125$. What we see is that our results are consistent with zero. We can see that there is a slight dependence of the error bars on the mass $m$. We have also checked, at the same value of the flux and bare mass, that as the temperature increases, the Haldane mass remains consistent with zero as to be expected from the solutions of the gap equations. However, when interpreting our results for the Haldane mass, one must take into account the taste-breaking terms which occur in the lattice action at $O(a)$ (see Appendix \ref{sec:SpinTasteAppendix}). The implication of this is that the spin degree of freedom in our lattice formulation is not conserved and thus interactions can flip the spin of a fermion.
...
\caption{The real part of the Haldane mass in units of magnetic flux as a function of the fermion bare mass $m$ for $8^2\times10\times480,\beta=0.80, \Phi_B=0.125$.}
\label{ReHaldanevsm} 
\end{figure}
...
&=& \gamma_1 \tilde{\gamma}_5 \Psi(-\vec{p}),
\eeq
...
In Euclidean space, the time-reversal transformation takes a different form. In this case, time is not distinguished from spatial coordinates by a relative minus sign in the metric. For example, in $(3+1)$ dimensions, time-reversal can be defined by the product of three successive reflection operations which reverse the sign of all the coordinates except one. In $(2+1)$ diemsnions, the time-reversal transformation on staggered fermions in the spin-taste basis is as follows
\beq
\nn
