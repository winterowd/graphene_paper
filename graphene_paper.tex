...
theory there are numerous short-range electron-electron interactions. These lattice scale interaction terms are consisten with the symmetry of the underlying hexagonal lattice \textcolor{red}{($C_{3v}$?)} \cite{Aleiner}.
Thus, these terms break the $U(4)$ symmetry present in our continuum EFT. The couplings associated with these terms can very in sign and are strongly renormalized at energies on the order of the bandwidth, $v_F/a_{hex}$ \cite{Kharitonov}.
Taking this into account can have a decisive effect on the selection of the ground state of the full theory. In our lattice gauge model, we only consider a long-range Coulomb interaction mediated by a scalar potential and ignore the Zeeman interaction as well as 
