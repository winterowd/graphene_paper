...
Finally, we study the mass spectrpoleiceum of the theory, in particular the Nambu-Goldstone (NG) mode as well as the Dirac quasiparticle, which is predicted to obtain a dynamical mass. 
\end{abstract}
...
\eeq
where $\epsilon$ is the dielectric constant of the substrate on which the graphene monolayer resides. The Dirac operator is given by 
\beq
...
\vev{ \chib \chi } = \frac{1}{V} \frac{1}{Z} \int \mathcal{D}U_0  \tr \left(\Dslash_{st} + m \right)^{-1}e^{-S^{\text{eff}}_E[U_0]},
\eeq
...
\begin{figure}
\vspace{-1cm}
\includegraphics[height=9cm,width=9cm]{pbp_vs_m_compare_graphene_paper.pdf} 
\vspace{-1.25cm}
...
\begin{figure}
\vspace{-1cm}
 \includegraphics[height=9cm,width=9cm]{pbp_vs_TdivM_PHI0125_zoom_graphene_paper.pdf} 
 \vspace{-1.25cm}
...
\begin{figure}
\vspace{-1cm}
  \includegraphics[height=9cm,width=9cm]{pbp_vs_m_PHI0125_zeroT_lin_graphene_paper.pdf} 
  \vspace{-1.25cm}
\caption{Chiral limit of the chiral condensate $\sigma$ using the $T=0$ extrapolated points. The errors on each point were determined from statistics as well as systematics. The value for the intercept is $0.2721(7)$, and the fit has $\chi^2/\text{d.o.f.} \approx 0.6$. The nonzero intercept indicates a finite value for the condensate in the chiral and zero-temperature limits.}
\label{PBPzeroTChiral}
\end{figure}

\begin{figure}
\vspace{-1cm}
 \includegraphics[height=9cm,width=9cm]{pbp_vs_PHI_graphene_paper.pdf} 
 \vspace{-1.25cm}
...
\vspace{-1cm}
  ...
  \vspace{-1.25cm}
\caption{The fermion dynamical mass as a function of the bare fermion mass for all four nonzero magnetic fluxes and zero flux.}
\label{MFvsmNonzeroB}
\end{figure}

As previously mentioned, perturbative approaches to magnetic catalysis predict that for $(2+1)$-dimensional field theories, the dynamical fermion mass scales lineary with $\sqrt{eB}$ \cite{Shovkovy}. In order to verify this statement, one first needs to extrapolate the dynamical mass to the chiral limit. Our results, depicted in Fig.~\ref{fig:MFvsSqrtB}, show that the chirally extrapolated dynamical mass behaves as expected. This gives further confirmation of magnetic catalysis in the graphene EFT.

\begin{figure}
\vspace{-1.2cm}
  \includegraphics[height=8.5cm,width=8.5cm]{ferm_mt_vs_sqrtPHI_linfit_graphene_paper.pdf} 
  \vspace{-1.25cm}
\caption{The zero-temperature, chirally extrapolated dynamical mass a function $\sqrt{eB}$. The linear fit depicted gives an intercept which is consistent with zero. This as the dynamical mass  is predicted to vanish in the chiral limit in the absence of an external magnetic field.}
\label{fig:MFvsSqrtB}
\end{figure}
