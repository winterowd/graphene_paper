...
Including short-range interactions in addition to the long-range Coulomb interactions brings forth a rich phase diagram with the existence of an antiferromagnetic (AFM) phase in addition to the semimetal and CDW phase \cite{Herbut, ITEPRealistic}. Adding an on-site Hubbard interaction and realistic nearest and next-nearest-neighbor interactions \cite{Wehling} to the model each of these phases to be accessed by varying the relevant couplings. In particular, graphene undergoes a transition to the AFM phase when the on-site interaction exceeds its critical value $U > U_{cr}$. Consistent with the results for the EFT mentioned above, for small $U$, one undergoes a transition from the semimetal phase to the CDW phase when $\alpha_g > \alpha^{cr}_g$. In this study, we perform calculations in the graphene EFT at half-filling with $U=0$, where one can construct a positive-definite, real action.  
...
\mathcal{H}_Z = -\mu_B B\int dt d^2x~ \Psi^{\dagger} \sigma_3 \Psi,
\eeq
...
\beq
P_{\sigma} \equiv \frac{1}{2}( 1 \pm \sigma_3 ).
\eeq
...
S^{(NC)}_G = a^3_s a_t \frac{(\epsilon+1)}{4e^2} \sum_n \sum^{3}_{i=1} \frac{1}{a^2_s}\left(A_0(n) - A_0(n+\hat{i})\right)^2,
\eeq
where $a_s$ and $a_t$ are the lattice spacings in the spatial and temporal directions respectively. Here $n=(n_0,n_1,n_2,n_3)$ labels the lattice site and $\hat{i}$ is the unit vector in the ith direction. Using the dimensionless field $\hat{A_0}(n) = a_t A_0(n)$, we write the above action as
\beq
\label{NCGaugeAction2}
S^{(NC)}_G = \frac{\beta}{2} \sum_n \sum^{3}_{i=1} \left(\hat{A_0}(n) - \hat{A_0}(n+\hat{i})\right)^2,
\eeq
where $\beta = \xi (\epsilon+1)/2e^2$ and $\xi \equiv a_s/a_t = 3v_F$ is the anisotropy parameter, controlling the ratio of the spatial lattice spacing to the temporal lattice spacing.
...
come closer to realizing the continuum theory. We point out, however, that we are not interested in explicitly taking the continuum limit as this corresponds to sending the cutoff to infinity while the EFT is only valid for long wavelengths.
...
G^{(\tau)}_F(\tau; \vec{p}) = \sum_{\vec{r}} e^{i \vec{p} \cdot \vec{r}}\vev{\chi(x,y,\tau) \chib(0,0,0)},
\eeq
...
The situation changes as one turns on the external magnetic field. Namely, the critical $\beta_{cr} = \xi (\epsilon_{cr}+1)/2e^2$ which determines the boundary between the two phases shifts to larger values. This has been shown previously in \cite{Polikarpov}, where at fixed temperature, the authors
...
We began our calculations by identifying the semimetal phase through a scan in $\beta$, coupled with an investigation of the condensate's behavior as a function of the bare mass in the chiral limit. At large values of $\beta$, which correspond to values of the graphene fine-structure constant which are less than the critical value of $\alpha^{cr}_g = 2e^2/(4\pi v_F (\epsilon_{cr} + 1)) \approx 1.1$ \cite{Drut1}, we expect to be inside the semimetal phase. In this symmetric phase, one expects $\sigma \equiv \vev{\Bpsi \Psi}$ to be linear in the bare mass for small values of $m$. One can compare this expectation with the behavior of $\vev{\Bpsi \Psi}$ as a function of the bare mass when the external magnetic field is turned on. This is illustrated in Fig.~\ref{PBPComparison}, where one sees an approximately linear behavior at zero field and a strongly nonlinear behavior of the condensate as a function of $m$ for the magnetic flux $\Phi_B = 0.125$. In our calculations we have fixed $\beta=0.80$, which corresponds to $\alpha_g \approx 0.3$. This further assures us that we are deep within the semimetal phase.
...
\acknowledgements
This work was in part based on a variant of the MILC collaboration's public lattice gauge theory code. See {\bf http://physics.utah.edu/$\sim$detar/milc\_qcd.html}.
...
\beq
\nn
\partial^2_{\mu} u(y) \equiv \frac{1}{4(2a)^2} ( u(y+2\hat{\mu}) &+& u(y-2\hat{\mu}) \\  &-& 2u(y) ).
\eeq
...
\beq
\tr{(\Gamma^{\dagger}_{\eta}\Gamma_{\eta'} + B^{\dagger}_{\eta}B_{\eta'})} = 4 \delta_{\eta \eta'}.
\eeq 
...
\bibitem{Herbut}
I.~F.~Herbut, Phys.\ Rev.\ Lett.\ {\bf 97}, 146401 (2006), [arXiv:cond-mat/0610349]

\bibitem{ITEPRealistic}
M.~V.~Ulybyshev, P.~V.~Buividovich, M.~I.~Katsnelson, and M.~I.~Polikarpov, Phy.\ Rev.\ Lett.\ {\bf 111}, 056801(2013), [arXiv:1304.3660 [cond-mat.str-el]]

\bibitem{Wehling}
T.~O.~Wehling, E.~Sasioglu, C.~Friedrich, A.~I.~Lichtenstein, M.~I.~Katsnelson, and S.~Bl\"ugel, Phys.\ Rev.\ Lett.\ {\bf 106}, 236805 (2011), [arXiv:1101.4007 [cond-mat.mes-hall]]
...
\bibitem{Rothe}
H.~J.~Rothe, {\it Lattice Gauge Theories: An Introduction}. World Scientific (2005)
