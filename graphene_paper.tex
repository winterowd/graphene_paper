...
\widetext
\leftline{Version 5.0 as of \today}
...
\label{ShiftOperator} 
 ...
 \sum_{y;\eta_{\mu} \neq \eta'_{\mu}} \chib_{\eta} \chi_{\eta'} \approx \frac{1}{N_v} \sum_{\eta} \sum^{N_v}_{k=1} \Phi^{(k) \dagger}_{\eta} M^{-1} \tilde{\Phi}^{(k)}_{\eta}.
 \eeq
...
At zero magnetic field, we expect the dynamical fermion mass to vanish in the chiral limit. In Fig.~\ref{MFvsmZeroB}, one can see our nonperturbative determination of the dynamical fermion mass plotted along with the location of the pole of the lattice fermion propagator at $O(e^2)$. The perturbative result explains the large renormalization of the mass as compared to the free theory. However, it appears that making an extrapolation to zero bare mass, based on both the perturbative and nonperturbative results, leaves us with a nonzero value for the dynamical fermion mass. Based on the arguments and results previously discussed, one expects the dynamical mass to vanish in this limit as we are firmly in the semimetal phase in the absence of the external magnetic field. Referring to Fig.~\ref{MFvsmZeroBZoom}, one can see that as one approaches the origin, the perturbative result shows large curvature and eventually vanishes. Using this result as a heuristic explanation of our results at zero magnetic field, one might expect the same to happen in the chiral limit for a nonperturbative calculation.
...
external magnetic field \cite{SmilgaShushpanov}. We also have determined that the pseudoscalar mode is indeed a bound state. This can be seen by referring to Fig.~\ref{MFvsmNonzeroB} where the dynamical fermion mass is plotted as a function of the bare mass. The fermion-antifermion scattering state has energy greater than $2m_F$ which is higher than $m_{\pi}$ for low bare masses.
...
the Haldane mass with staggered fermions. The evidence for a dynamically generated Dirac mass for the quasiparticle as well as the nonzero value for the chirally extrapolated, $T=0$ chiral condensate shows
...
\eeq
where, for a fermion bilinear of the form $\Bpsi A \Psi$, due to the fact that time-reversal, $\mathcal{T}$, is an anti-unitary operator, $\mathcal{T} A \mathcal{T}^{-1} = A^*$.
In Euclidean space, the time-reversal transformation takes a different form. In this case, time is not distinguished from the spatial coordinates by a relative minus sign in the metric. For example, in $(3+1)$ dimensions, time-reversal can be defined by the product of three successive reflection operations which reverse the sign of all the coordinates except one and does not involve complex conjugation. In $(2+1)$ dimensions, the time-reversal transformation on staggered fermions in the spin-taste basis is as follows
\beq
\nn
